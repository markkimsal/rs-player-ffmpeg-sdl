...
use gl::{types::*, TextureStorage2D, TEXTURE_2D};
use libc::suseconds_t;
use rusty_ffmpeg::ffi::{self, av_frame_unref};

use sdl2::{
    event::Event, keyboard::Keycode, pixels::{Color, PixelFormatEnum}, render::{Canvas, Texture, TextureAccess}, sys::{SDL_GL_BindTexture, SDL_GL_SwapWindow, SDL_Texture, SDL_UpdateYUVTexture}, video::Window, Error, Sdl
};
...
pub fn init_subsystem<'sdl>(default_width: u32, default_height: u32) ->Result<SdlSubsystemCtx, Error> {
    ...
    gl::load_with(|p| video_subsystem.gl_get_proc_address(p) as *const _);
...
pub unsafe fn event_loop(movie_state: &mut movie_state::MovieState, subsystem: &mut SdlSubsystemCtx, tx: std::sync::mpsc::Sender<String>) {
    ...
    'running: loop {
        ...
        //draw opengl
        draw_opengl(subsystem.canvas.window(), texture.raw());
    }
...
unsafe fn draw_opengl(window: &sdl2::video::Window, texture: *mut SDL_Texture) {
         ...
         */
        // let v0: Vec<GLfloat> = [-1.0f32, -1.0f32,  1.0f32];
        ...
        gl::BufferData(gl::ARRAY_BUFFER, (3 * 4 * 2) as _, ::std::ptr::null(), gl::DYNAMIC_DRAW);
        ...
        SDL_GL_BindTexture(texture, 1280 as _, 720 as _);
        // gl::TexCoordP1ui(gl::QUADS, coords);

        // gl::DrawArrays(gl::QUADS, 0, 3);
        ...
        //  */
        // SDL_GL_SwapBuffers( );
        SDL_GL_SwapWindow(window.raw() as *mut _);
    }
