...
pub struct SdlSubsystemCtx {
    sdl_ctx: RaylibHandle,
    thread: RaylibThread,
    is_recording: bool,
}

pub fn init_subsystem<'sdl>(default_width: u32, default_height: u32) ->Result<SdlSubsystemCtx, std::io::Error> {

    let (mut rl, thread) = raylib::init()
        .size(640, 480)
        .title("rs-ffmpeg-player")
        .build();
...
pub unsafe fn event_loop(movie_state: std::sync::Arc<&mut movie_state::MovieState>, subsystem: &mut SdlSubsystemCtx, tx: std::sync::mpsc::Sender<String>) {
    ...
    subsystem.sdl_ctx.set_target_fps(60);

    let img = raylib::core::texture::Image::load_image("resources/rust.png").unwrap();
    let tex = subsystem.sdl_ctx.load_texture_from_image(&subsystem.thread, &img).unwrap();
    dbg!(&tex);
    ...
    while !subsystem.sdl_ctx.window_should_close() {
        let mut d = subsystem.sdl_ctx.begin_drawing(&subsystem.thread);
        d.clear_background(Color::LIGHTBLUE);

        d.draw_texture(
            &tex,
            ((screenw/2) as i32 - tex.width/2) as _,
            ((screenh/2) as i32 - tex.height/2) as _,
            Color::WHITE
        );
